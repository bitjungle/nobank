...
# Kolonner som trygt kan konverteres til int
int_columns = {"LEMMA_ID", "PARADIGME_ID", "BOY_NUMMER", "LEDDANALYSE_ID"}

# Kolonner som er datoer
...
try:
    for file_path, table in files.items():
        ...
        print(f"📥 Importerer {file_path} → {table}")
        df = pd.read_csv(
            file_path,
            sep="\t",
            dtype=str,
            keep_default_na=False, # <-- beholder 'null' og 'nan' som strenger
            na_values=[]           # <-- ingen automatisk NaN-tolkning
        )

        df.columns = [col.strip("'").strip() for col in df.columns]

        # Fjern LOEPENR om den finnes
        df.drop(columns=[col for col in df.columns if col.upper() == 'LOEPENR'], inplace=True, errors='ignore')

        # Rens og konverter datatyper
